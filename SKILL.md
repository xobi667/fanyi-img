---
name: xobi-img
description: 跨 Codex、OpenClaw 与 AgentSkills 兼容宿主的通用位图生产技能，提供生成、编辑、fanyi 图片翻译、文字替换、换背景、对象增删、合成、修图、透明背景、比例转换、Logo、主图、多变体、ZIP、最多四路批处理和最终压缩。普通生成与编辑继续使用当前规则；翻译恢复 531e28e 的 fanyi 方式：当前源图作为唯一参考，只替换原有文字，1:1 默认最终优化为 800×800 JPG、900–1024KB。用户只给素材但没说改什么、没给比例，或翻译没给目标语言时，先只询问缺失项。
---
...
- `commerce_main_image`：用户已明确要求制作、重做或优化整张主图，并确认目标商品/素材、平台或“通用电商”、视觉方向、输出比例和文字策略。
...
3. `localization`：执行恢复后的 `fanyi` 参考图翻译。把当前源图作为唯一编辑参考，只替换原图已有文字；不使用无参考整张重建，也不运行文字框本地拼贴。完整规则见 [references/localization.md](references/localization.md)。
4. `commerce_main_image`：显式 creative route。只有用户明确要求制作、重做或优化整张主图时，才按冻结艺术指导重做呈现；无输入仍以 `generate` 预检，有输入仍以 `edit` 预检。完整规则见 [references/main-image.md](references/main-image.md)。
5. `batch`：把独立的 generate/edit/localization/commerce_main_image 任务分给不同 worker；不是四个人重复处理同一张图。

“翻译主图”、给主图换背景、替换一处文字或修改某个对象都不自动进入 `commerce_main_image`。单独翻译仍只替换已有文字，普通 edit 仍只改点名项；不得因为图片像电商图、源图不好看或用户使用了“主图”这个名词就静默扩大权限。主图 route 的创意授权只覆盖已冻结的构图、背景、光影、色彩关系、信息层级和文字视觉，不得改商品身份、数量、型号、颜色、部件、真实结构或用户事实。

用户已给同语言精确替换文案（如 `SALE → SOLD OUT`）时路由为 `edit/text_replacement`，不强制追问目标语言；只有需要跨语言翻译时才使用 localization 门禁。

用户说“翻译”时是死命令：把当前源图传给内置图片编辑能力，只把已有文字替换为准确译文。商品、照片、人物、图标、Logo、边框、色块、背景、阴影、纹理、顺序、数量、位置、间距、构图和版式一律不许改；无字区域继续无字。译文只能在原文字模块内做必要换行、字号和字距适配，不能添加角标、卖点、装饰或伪字。
...
翻译不再使用 `pure_generation_localization`。每张图每次都从原始源图重新参考编辑，最多 3 个有候选质量尝试；不得把失败候选作为下一次参考。
...
- generate、普通 edit 和 `commerce_main_image` 继续遵守各自现有的纯生图规则。localization 单独恢复 fanyi：本地源图用 `referenced_image_paths`，仅会话图片才用最小 `num_last_images_to_include`，两者不得共存。
- 每个 fanyi attempt 只传当前 task 的一张源图并只产一张候选；不得附带另一张源图、失败候选、Logo 素材或无关最近图片。候选通过逐图验收后才进入最终压缩。
...
- 本节只在用户明确要求添加 Logo 时启用。源图已有 Logo 不等于用户要求添加 Logo；翻译仍只使用当前源图参考，不能附带 active Logo。
- 添加 Logo 的开工必填项包括本次 Logo 资产及其 `logo` 角色。只有用户明确说“使用默认 Logo”时才能启用技能内置模板；不得因用户没给 Logo 就静默套用旧 Logo。
- 用户只要求添加 Logo 时，不先执行 `pure_generation_edit`：源图或用户明确要求的尺寸/格式转换结果就是待叠加基底。组合翻译任务先完成 fanyi 翻译候选验收；第一阶段禁止让模型生成、临摹或加入本次 active Logo。
...
- `logo_conflict` 每次返回可读重排候选时，都必须用当前全局 attempt+1 登记唯一且保留在 `.xobi/work` 的 `--prepared-base`；记录其路径、SHA-256 和尺寸，最多 3 个质量候选且不得复用候选路径或哈希。没有候选的基础设施 attempt 禁止携带 `prepared_base`、output 或其他候选产物。Logo 冲突候选一旦要登记为 accepted，必须在同一次 update 复算通过 relocation/pixel-lock，并把完全相同的证据绑定到 item 与 attempt；不能等最终 Logo success 才检查。主图或 Logo 冲突候选一旦验收通过，对应图片阶段立即封口，不得追加同阶段 attempt。
...
- 批量翻译运行 `scripts/preflight_fanyi.py`；其他模式继续运行 `scripts/preflight_images.py`。翻译的源图、worker、原始候选和最终压缩输出必须一一对应。
- 默认 `MAX_WORKERS = min(4, 宿主可用槽位, 待处理数, 宿主并发上限)`。宿主明确不支持并行时直接设置 `workers=1`；未知时默认最多 4。
...
- Fanyi 翻译每次调用连续记录，返回候选即占用一次质量尝试；初次结果后最多 2 次针对性重试。没有候选的基础设施错误最多 4 次并按 2/5/10 秒退避。成功图不重跑，只补失败或缺失图。
...
- 两个 worker 出现同类基础设施错误时停止新并行调用并降为单路；fanyi 单路继续沿用当前 task 的原始源图、prompt 和隔离输出，不创建无归属探针。
...
- `commerce_main_image` 批量是明确例外：按平台、比例、品类、视觉方向和文字策略分 family，每个 family 必须先有内部 pilot 通过全尺寸/长边 256/长边 160 三档验收，再并行其余成员；这是内部质量门禁，不要求用户逐张确认。
...
- 电商主图唯一规则：[references/main-image.md](references/main-image.md)
...
- `scripts/create_contact_sheet.py`：普通总览；翻译使用 source/fanyi_raw_candidate/final，Logo 使用 source/conflict_reference_base/prepared_base/final 分阶段对照。
- `scripts/create_main_image_review.py`：先 prepare 当前主图候选原始字节的独立 full snapshot 与保持比例长边 256/160 绑定证据，再 finalize 人工三档审美结果；每个 `commerce_main_image` 有候选 attempt 都必须在登记前运行，不能只在 success 前补跑。
- `scripts/preflight_fanyi.py`：恢复自 `531e28e`，负责 fanyi 批量盘点、四路分工、已有成品检查和原始/最终目录规划。
- `scripts/final_optimize_images.py`：恢复自 `531e28e`，负责 fanyi 的最终 `800×800 JPG、900–1024KB` 压缩交付。
- `scripts/compose_localization.py`：不用于恢复后的 fanyi 翻译。
...
1. 只读查看输入并通过开工门禁。
2. 读取当前模式所需规则文件；主图必须读取 [references/main-image.md](references/main-image.md)；批量创建 manifest 并锁定输入角色。
3. 翻译先运行 fanyi 预检并建立逐图文字锁；其他模式继续使用现有计划与 manifest。
4. Fanyi worker 每次把当前源图作为唯一参考完成翻译；普通生成、编辑、主图和 Logo 不改变现有执行方式。
5. 逐图查看 source/raw/final，确认只换文字；1:1 再运行最终压缩并复核尺寸、格式、体积和数量。
