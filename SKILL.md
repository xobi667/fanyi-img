---
name: "fanyi"
description: "保留商品图翻译触发范围，并补充统一排版与系列风格能力。"
---
...
- 2 张或以上、文件夹、压缩包或飞书多图任务：最终成品打包成功后，只发送打包好的 ZIP，不再额外逐张发送图片。
...
- ZIP 超过飞书限制时，优先分卷或上传飞书云盘并发送链接，不得删减成品。
- 飞书交付 ZIP 或成品图时，`message(action="send")` 只附加文件，不在工具调用中填写重复说明文字；发送回执成功后，由普通 final 只发送一次简短完成确认。
...
当用户明确说“统一 / 统一排版 / 同一套风格 / 系列感 / 保持一致”时，必须启用批量视觉系统锁；未明确要求时仍优先保持每张源图原有版式。

- 生图前先根据整批源图建立一份共享的 `BATCH_STYLE_LOCK`，至少记录：目标画布、字体风格、标题/正文层级、字号范围、文字对齐、文字框区域、外边距、间距、色板、图标/标签样式和商品视觉尺度范围。
- `BATCH_STYLE_LOCK` 只共享视觉规范，不共享任何图片的商品、文字块、翻译、参数、构图细节或会话图片。
- 每张图仍必须使用自己的 `task_id`、文字锁和商品几何锁；内容隔离与批次风格统一必须同时满足。
- 禁止各 worker 自行发明不同版式。主协调者建立并下发同一份 `BATCH_STYLE_LOCK`，所有 worker 原样遵守。
- 保留每张源图的信息结构和卖点区域；“统一”不代表把所有图片改成同一个模板，也不允许新增、删除或串用文字。
- 批量完成后必须制作或查看整批缩略图总览，横向检查字体、层级、位置、边距、间距、色彩、商品尺度和产品形变；明显脱离批次视觉系统的图片必须重生。
- 用户说“重新统一 / 统一排版重做”时，默认重做整批，不只补单张，除非用户明确限定具体图片。
...
8. 输出后按 `references/quality.md` 做基础质检；用户要求统一时额外做整批缩略图横向质检，失败则重生。
