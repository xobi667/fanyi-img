---
name: xobi-img
description: 跨 Codex、OpenClaw 与 AgentSkills 兼容宿主的通用位图生产技能，默认使用当前宿主的原生图片模型以不传参考图的纯生图方式完成生成、翻译、文字替换、换背景、增删对象、换色、合成、修图、透明背景、比例转换、多变体、ZIP 和最多四路批处理；只有用户明确要求制作、重做、重新设计或整体优化整张主图时才启用 creative commerce_main_image，翻译主图、局部改字、换背景、换 Logo 或只检查主图都不启用，普通编辑只改用户点名项，翻译只替换已有文字。唯一例外是用户明确要求添加 Logo：无冲突时本地确定性叠加真实 Logo，有冲突时允许 Logo 专属局部参考重排后再叠加。用户只给图片、文件夹或压缩包但没说改什么时先询问；所有图片任务的输出比例未明确时先询问；翻译缺少目标语言时先询问。信息完整后直接执行。
---
...
- generate、edit、localization 和 `commerce_main_image` 的原生图片模型调用默认都不传参考图、最近会话图片或参考图参数；输入型任务先由协调者查看源图/素材并把必要内容完整写入当前 task 的纯生图 prompt。只有用户明确要求添加 Logo 并确认本次 `logo` 资产时，才可启用 Logo 专属例外；源图本来含有 Logo、盘点清单登记了 Logo 或用户只要求翻译/普通编辑，都不构成例外。唯一可使用参考编辑的是该 Logo 添加任务中的冲突底图重排，且只在 [references/logo.md](references/logo.md) 规定的冲突任务内使用。
- 每个 attempt 只调用一次图片工具，至多产生当前任务的一个候选；候选通过验收后直接作为成品视觉内容。`commerce_main_image` 的每个有候选 attempt 还必须先完成独立 full snapshot、256/160 缩略图、assessment 和 finalized review，并在同一次 manifest update 绑定该 review；失败候选也必须绑定 `passed=false` review。除已明确启用的最终真实 Logo 确定性叠加外，不得再用本地蒙版、裁贴或局部合成修改。基础设施失败可以没有候选，也不得登记 candidate/review。质量或基础设施失败可按重试策略新建 attempt；不同任务不得共享会话图片、prompt、输出路径或图片上下文。
...
3. 建立逐图内容锁；翻译在 attempts=0 时用独立 pending 更新登记并冻结 `pure_generation_localization` plan；主图冻结艺术指导并先做 family 内部 pilot；Logo 写 logo plan/family pilot。
4. 按 task 分工执行。默认原生图片调用不传参考图，每个 attempt 只产一个完整纯生图候选；主图候选先完成并冻结该 attempt 的 review 再连同结果一起登记，其他模式按各自验收后立即记录。只有 Logo 冲突底图可进入专属参考编辑例外。
5. 逐图查看 source/candidate/final；普通编辑核对只改点名项，翻译核对只换已有文字；主图查看独立 full snapshot/长边 256/长边 160，内容正确但审美不合格也必须保留 `passed=false` review 并按精确原因重试。
