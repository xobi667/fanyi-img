...
- 生图阶段不要本地二次改比例；最终优化阶段按 800x800 收尾规则处理。
...
4. 批量任务先运行 `scripts/preflight_fanyi.py`。
5. 扫描文件，但跳过输出目录、原始生图目录和其他语言输出目录。
6. 创建原始生图目录和最终输出目录，并保持相对路径结构。
...
16. 用户要求 1:1 时检查所有图片输出尺寸，无文字商品图/纯产品图/空白图也必须检查。
17. 翻译全部完成后，生成基础报告和失败清单。
18. 除非用户明确禁止最终优化，否则运行 `scripts/final_optimize_images.py`。
19. 检查最终优化报告，确认最终图片为 800x800 JPG，且体积在 900KB-1024KB。
