...
11. 强制使用 `MAX_WORKERS = 1`，一张一张顺序处理，禁止并发。
12. 失败自动重试，但必须在当前图片内部完成重试。
13. 用户要求 1:1 时检查所有图片输出尺寸，无文字商品图/纯产品图/空白图也必须检查。
14. 翻译全部完成后，生成基础报告和失败清单。
15. 除非用户明确禁止最终优化，否则运行 `scripts/final_optimize_images.py`。
